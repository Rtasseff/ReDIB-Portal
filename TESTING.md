...
|------|--------|------|-----------------|
| 0.1 | Login to Django admin at `/admin/` | `admin` (superuser) | Access admin panel |
...
| 0.4 | Verify users populated | `admin` | See 8 core users (coordinators, evaluators, applicants) |
| 0.5 | Check user roles in Core > User Roles | `admin` | Verify coordinator, evaluator (with areas), applicant roles |
| 0.6 | Verify email templates seeded | `admin` | Communications > Email Templates shows 10+ templates |

**Key data to verify:**
- Nodes: CIC biomaGUNE (code: CICBIO), TRIMA@CNIC (code: CNIC)
- Equipment: MRI 7T, PET-CT, etc. with proper node assignments
- Evaluators have area qualifiers: `evaluator:preclinical`, `evaluator:clinical`, `evaluator:radiotracers`
...
| Step | Action | User | Expected Result |
|------|--------|------|-----------------|
| 1.1 | Login at `/` | `coordinator@redib.es` | Redirect to coordinator dashboard |
| 1.2 | Navigate to Calls section | `coordinator` | See list of calls (if any exist) |
| 1.3 | Create new call | `coordinator` | Click "New Call" button |
...
| 1.11 | Navigate to call detail | `coordinator` | See call summary |
| 1.12 | Add equipment allocation | `coordinator` | Select equipment (e.g., MRI 7T at CIC biomaGUNE) |
...
| 1.15 | Publish call | `coordinator` | Status → `open`, `published_at` timestamp set |
...
- ✅ Cannot publish call without at least one equipment allocation
- ✅ Published call (status=`open`) visible to anonymous users
- ✅ Draft calls NOT visible to public
